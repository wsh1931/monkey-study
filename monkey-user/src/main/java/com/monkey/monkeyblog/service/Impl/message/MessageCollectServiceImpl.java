...
public class MessageCollectServiceImpl implements MessageCollectService {
     ...
     * @return {@link null}
     * @author wusihao
     * @date 2023/10/29 21:52
     */
    @Override
    public R queryCollectMessage(long userId, Long currentPage, Integer pageSize) {
        QueryWrapper<MessageCollect> messageCollectQueryWrapper = new QueryWrapper<>();
        messageCollectQueryWrapper.eq("recipient_id", userId);
        messageCollectQueryWrapper.orderByDesc("create_time");
        Page page = new Page<>(currentPage, pageSize);
        Page selectPage = messageCollectMapper.selectPage(page, messageCollectQueryWrapper);
        List<MessageCollect> messageCollectList = selectPage.getRecords();
        // 最终返回集合
        List<MessageVo> messageVoList = new ArrayList<>();
        // 查询到的未读消息id集合
        List<Long> messageIdList = new ArrayList<>(messageCollectList.size());
        for (MessageCollect messageCollect : messageCollectList) {
            if (messageCollect.getIsRead().equals(CommonEnum.MESSAGE_NOT_READ.getCode())) {
                messageIdList.add(messageCollect.getId());
            }
            MessageVo messageVo = new MessageVo();
            BeanUtils.copyProperties(messageCollect, messageVo);
        ...
        if (messageIdList.size() > 0) {
            ...
            jsonObject.put("event", EventConstant.updateCollectMessageAlready);
     ...
     */
    @Override
    public R deleteCollectMessage(Long messageId) {
        return R.ok(messageCollectMapper.deleteById(messageId));
    }
     ...
     */
    @Override
    public R updateAllCollectAlready(String userId) {
        if (userId == null || "".equals(userId)) {
            return R.error(ExceptionEnum.NOT_LOGIN.getMsg());
        }

        UpdateWrapper<MessageCollect> messageCollectUpdateWrapper = new UpdateWrapper<>();
        messageCollectUpdateWrapper.eq("recipient_id", userId);
        messageCollectUpdateWrapper.set("is_read", CommonEnum.MESSAGE_IS_READ.getCode());
        messageCollectMapper.update(null, messageCollectUpdateWrapper);
        return R.ok();
    }
     ...
     * @return {@link null}
     * @author wusihao
     * @date 2023/10/29 22:03
     */
    @Override
    public R deleteAllCollectMessageOfAlreadyRead(String userId) {
        if (userId == null || "".equals(userId)) {
            return R.error(ExceptionEnum.NOT_LOGIN.getMsg());
        }
        QueryWrapper<MessageCollect> messageCollectQueryWrapper = new QueryWrapper<>();
        messageCollectQueryWrapper.eq("recipient_id", userId);
        messageCollectQueryWrapper.eq("is_read", CommonEnum.MESSAGE_IS_READ.getCode());
        messageCollectMapper.delete(messageCollectQueryWrapper);

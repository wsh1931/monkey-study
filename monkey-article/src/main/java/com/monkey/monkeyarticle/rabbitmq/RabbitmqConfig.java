...
public class RabbitmqConfig {
    ...
    // 定义文章添加直连交换机
    @Bean
    public DirectExchange articleAddDirectExchange() {
        return ExchangeBuilder.directExchange(RabbitmqExchangeName.articleInsertDirectExchange).build();
    }

    // 定义文章添加直连死信交换机
    @Bean
    public DirectExchange articleAddDlxDirectExchange() {
    ...
    // 文章直连交换机绑定添加数据队列
    @Bean
    public Binding articleDirectDlxExchangeBindingInsertQueue(DirectExchange articleAddDirectExchange, Queue insertQueue) {
        return BindingBuilder.bind(insertQueue).to(articleAddDirectExchange).with(RabbitmqRoutingName.articleInsertRouting);
    }

    // 文章直连死信交换机绑定添加死信队列
    @Bean
    public Binding articleDlxDirectExchangeBindingInsertDlxQueue(DirectExchange articleAddDlxDirectExchange, Queue insertDlxQueue) {
        return BindingBuilder.bind(insertDlxQueue).to(articleAddDlxDirectExchange).with(RabbitmqRoutingName.articleInsertDlxRouting);
    }
